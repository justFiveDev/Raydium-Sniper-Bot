...
import { BorshCoder, EventParser, Idl as CoralIdl } from "@coral-xyz/anchor";
  ...
  constructor(programInfos: ProgramInfoType[], private logger: Console) {
    ...
    for (const programInfo of programInfos) {
      this.addParserFromIdl(
        ...
        programInfo.idl as CoralIdl
      );
    }
  }

  addParserFromIdl(programId: PublicKey | string, idl: CoralIdl) {
    if (idl?.events) {
      try {
